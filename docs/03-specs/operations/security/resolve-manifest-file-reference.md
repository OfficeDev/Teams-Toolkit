...
| FILE-AC-10 | L1      | operation-integration | required | TempDirRuntime | A file reference resolves lexically or canonically outside the manifest directory         | The error is surfaced in VS Code | The lifecycle summary identifies the reference and remediation without directing the user back to the Output panel; local output also identifies the resolved target and manifest directory, while the telemetry-facing error omits paths. |
