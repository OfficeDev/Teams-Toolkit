...
| ZIP-AC-04  | L1      | operation-integration | required | TempDirRuntime | A package source is rejected for leaving the trusted directory                            | The error is surfaced     | The error and log output do not disclose a resolved absolute host path.                      |
...
3. Containment handles parent traversal, sibling-prefix paths, symbolic links, and Windows
   cross-drive paths.
4. Error messages do not expose resolved local absolute paths.
