...
| WORKER-VALIDATE-06 | L1      | operation-integration | required | TempDirRuntime | Duplicate IDs, ReferenceKeys, Windows case aliases, or distinct paths resolving to one canonical target exist | Validate runs | Stable duplicate diagnostics are returned for later occurrences.                                             |
...
flowchart TD
  ...
  F --> G[Detect duplicates, self edges, cycles, and depth]
