...
Returns `Result<WorkerMutationResult, FxError>`, where `WorkerMutationResult` contains
`changed: boolean`.
...
| WORKER-ADD-02 | L1      | operation-integration | required | TempDirRuntime | A v1.8 root DA and an existing regular JSON DA file inside appPackage                                                                                            | Add runs | One authored `{ "file": value }` entry is appended and `changed` is true.                                                           |
| WORKER-ADD-03 | L1      | operation-integration | required | TempDirRuntime | An equivalent ID, ReferenceKey, or canonical file target already exists                                                                                          | Add runs | Manifest bytes are unchanged and `changed` is false.                                                                                |
...
flowchart TD
  ...
  C --> D{File reference?}
  D -- yes --> E[Resolve regular canonical target and containment]
