...
| VCB-162 | Given a checked `da/no-action` project, workflow version `v1.9`, Microsoft 365 login, and specified-users `share` with an environment-backed email, compilation verifies the top-level version mutation, verifies the Command Palette lists `Microsoft 365 Agents: Remove access to the shared app` first and `Microsoft 365 Agents: Share` second, selects Share, selects `dev` from the environment prompt, completes the coordinate-free Share questions, and asserts the unsupported-version notification. The manual case authors the four `da/no-action` scaffold answers in prompt order, replaces `DA_Error_Message_of_Legacy_Projects.json`, and preserves the VSIX-to-engine compatibility contract without installing historical extension versions.                                                                                                                                                                         |
