...
| `target.with.entry`                                     | Optional closed launch entry: `command-palette` or `workbench`; defaults to `command-palette`. The Workbench entry is supported only where a profile-specific recorded adapter exists.                                                |
...
| `target`    | Use the declared compiler-owned entry adapter to select and start the exact authored profile; assert the profile adapter's target readiness               | Profile-owned target readiness |
...
| VCB-123 | Given a Simple Bot local Teams `target` with `entry: workbench`, compilation uses the profile-specific Run and Debug Workbench recording to select and start `Debug in Teams (Chrome)`, then reuses the existing profile authentication, readiness, Teams activation, and chat adapters; omitting `entry` keeps the Command Palette path, while an unknown or unsupported entry/profile combination fails before plan output.                                                                                                                                                      |
