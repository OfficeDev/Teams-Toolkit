...
| VCB-43  | Given a Copilot target profile, its readiness assertion requires the main section to show the opened agent's identity header with an icon and display name together with a visible message input, proving an agent chat is open without comparing the generated app name. Other target surfaces retain their own readiness subjects.                                                                                                                                                                                                                                                                                                 |
...
| VCB-125 | Given Microsoft 365 Copilot can preserve underscores in one displayed agent name and omit them in another, neither target readiness nor the message-input assertion transforms or compares the generated app name; readiness instead requires the opened agent's identity header and message input, while a `chat-ready` message step requires only that agent chat's visible input.                                                                                                                                                                                                                                                 |
