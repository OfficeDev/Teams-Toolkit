...
| `steps.<name>.type`                                     | Required; one of `scaffold`, `login`, `provision`, `deploy`, `pythonEnvironment`, `localEnvironment`, `playgroundEnvironment`, `localUserEnvironment`, `removeWorkspaceFile`, `configureTypeSpecAction`, `target`, `open`, or `checks`. |
...
`configureTypeSpecAction`, `target`, `open`, and `checks`.
...
| VCB-160 | Given the Weather Agent TypeScript and JavaScript OpenAI Playground cases, each endpoint redirect uses a `playgroundEnvironment` operation that adds `OPENAI_BASE_URL` to the `envs` mapping in `m365agents.playground.yml` which writes `.localConfigs.playground`; the generated mutation must not target `m365agents.local.yml`, because the Playground launch does not consume `.localConfigs`.                                                                                                                                                                                                                                                                                                                                                                  |
