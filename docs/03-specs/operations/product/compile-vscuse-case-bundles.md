...
| VCB-98  | Given a General Teams Agent case that targets Copilot, its plan alone declares `TEAMSFX_CEA_ENABLED=true` for injection before the container and VS Code extension host start; extension activation preserves that explicit environment override, and the plan writes no VS Code setting and reloads no window. General Teams Agent cases that target Teams or Agents Playground declare no such flag.                                                                                                                                                                             |
