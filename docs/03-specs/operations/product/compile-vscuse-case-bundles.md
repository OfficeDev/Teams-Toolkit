...
| `steps.<name>.type`                                     | Required; one of `scaffold`, `login`, `provision`, `deploy`, `pythonEnvironment`, `localEnvironment`, `playgroundEnvironment`, `localUserEnvironment`, `removeWorkspaceFile`, `workflowVersion`, `configureTypeSpecAction`, `share`, `target`, `open`, or `checks`. |
...
`localEnvironment`, `playgroundEnvironment`, `localUserEnvironment`, `removeWorkspaceFile`,
`workflowVersion`, `configureTypeSpecAction`, `share`, `target`, `open`, and `checks`.
...
environment-backed email, and asserts the complete unsupported-workflow-version notification. All
quick inputs use their existing coordinate-free components. Because the version gate runs before
package and remote-service access, the case does not provision and does not call the sharing service.
...
| VCB-154 | Given the seven named legacy plans, six new lowercase `feature-*.yml` sources and one existing `non-sso-tab.yml` case generate complete replacements, and their mapping rows are Full. The mislabeled `DA_No_Action_Add_Knowledge_Onedrive.json` and obsolete generic `DA_Add_Action_Import_Existing_API.json` plans are removed with Retired mapping rows because their retained behavior is covered elsewhere. `Basic_Tab_Remote_Debug.json` remains the single Partial case because the generated case omits the compiler-invalid provision-before-login path; four other blocked plans remain Not Mapped with evidence-backed blockers, and generation is idempotent.                                                                                                                                                                                                                                                               |
...
| VCB-171 | Given Microsoft 365 sign-in starts from either the email form or the returning-account picker, password submission is not treated as completed authentication: compilation emits a browser assertion that waits for the Visual Studio Code sign-in confirmation page before closing Chrome, and only then asserts that the requested account is ready in the toolkit side bar. Existing recorded pointer coordinates and entry-state routing remain unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
