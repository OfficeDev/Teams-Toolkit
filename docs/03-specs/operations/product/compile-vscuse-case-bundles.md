...
| `steps.<name>.type`                                     | Required; one of `scaffold`, `login`, `provision`, `deploy`, `pythonEnvironment`, `localEnvironment`, `localUserEnvironment`, `removeWorkspaceFile`, `configureTypeSpecAction`, `target`, `open`, or `checks`.                        |
...
| `configureTypeSpecAction.with.action`                   | Required literal `github-issues`. The adapter owns `src/agent/main.tsp` and the exact transformation; authors cannot provide a path, line number, or source text.                                                                     |
...
`localEnvironment`, `localUserEnvironment`, `removeWorkspaceFile`, `configureTypeSpecAction`,
...
`configureTypeSpecAction` accepts only `with.action: github-issues`. Its component opens a new
integrated terminal and types one compiler-owned command that targets the generated project's
absolute path. The command requires the disabled GitHub issues declarations to occur exactly once
in `src/agent/main.tsp`, enables only those declarations without relying on line numbers, re-reads
the file to verify the enabled form, and prints a unique success marker only after verification.
...
| VCB-34  | Given the checked-in case sources and no injected `compileStep`, setup reads no external template contracts and uses the semantic compiler plus component renderer to emit 138 deterministic current-format runnable plans; every operation resolves through a supported adapter, removed manifest-owned cases are deleted, and a second setup reports no generated-plan changes.                                                                                                                                                                                                                                                                                                  |
...
| VCB-43  | Given a Copilot target profile, its readiness assertion requires an agent to be selected in the `Agents` list and that agent's chat to be open in the main section with a visible message input, proving an agent chat is open without comparing the generated app name or requiring an icon in the main section. Other target surfaces retain their own readiness subjects.                                                                                                                                                                                                                                                                                                       |
...
| VCB-104 | Given the image build checks out a source ref that differs from the workflow dispatch ref, its source-ref tag, source-SHA tag, and OCI revision label identify the checked-out ref and commit; after publishing, verification by digest rejects the image unless that revision matches and the ATK feature-flag entrypoint and synchronization script are present, so consumers cannot mistake stale image infrastructure for the requested source.                                                                                                                                                                                                                                |
...
| VCB-113 | Given a local Teams Agent with Data case on the OpenAI branch, its chat check asserts no grounded answer, because the generated project hardcodes an OpenAI model name that the redirected Azure OpenAI resource does not host as a deployment; the local Azure OpenAI branch keeps the error-free reply assertion.                                                                                                                                                                                                                                                                                                                                                                |
...
| VCB-132 | Given the retained Basic Custom Engine Agent scenarios, TypeScript and JavaScript with OpenAI preserve the legacy fake key through `${{secret:FAKE_OPENAI_API_KEY}}` and reply-only checks across remote and local Teams and Copilot, while TypeScript and JavaScript with Azure OpenAI preserve the legacy error-free reply checks across remote and local Copilot; all twelve cases use existing scaffold, lifecycle, target, open, and chat-check operations, so their hand-recorded plans can be replaced without adding an engine capability.                                                                                                                                 |
...
| VCB-134 | Given the retained RAG Customize remote Copilot scenarios, each TypeScript, JavaScript, and Python case provisions and deploys before selecting `Launch Remote in Copilot (Chrome)`; OpenAI cases preserve the legacy fake key and reply-only prompt, while Azure OpenAI cases preserve the legacy history prompt and error-free reply contract. All six cases declare the Copilot feature flag and compose existing operations, so their hand-recorded plans can be replaced without adding an engine capability.                                                                                                                                                                 |
| VCB-135 | Given the retained Custom API Azure OpenAI remote Teams scenarios, each TypeScript, JavaScript, and Python case preserves its legacy work item, provisions and deploys before selecting `Launch Remote in Teams (Chrome)`, sends its language-specific legacy repairs prompt, and asserts an error-free reply; the Python case also creates its virtual environment. All three cases compose existing scaffold, file-check, lifecycle, target, open-app, and chat-check operations, so their hand-recorded plans can be replaced without adding an engine capability.                                                                                                              |
| VCB-136 | Given the retained Custom API OpenAI remote Teams scenarios for TypeScript and JavaScript, each case preserves its legacy work item and fake OpenAI key through `${{secret:FAKE_OPENAI_API_KEY}}`, provisions and deploys before selecting `Launch Remote in Teams (Chrome)`, sends `List all repairs without auth`, and asserts only that the bot replied, matching the legacy contract. Both cases compose existing operations, so their hand-recorded plans can be replaced without adding an engine capability.                                                                                                                                                                |
...
| VCB-139 | Given the retained RAG Customize remote Teams scenarios, TypeScript and JavaScript with OpenAI preserve the legacy fake key and `Compare Contoso Electronics plan` reply-only contract, TypeScript and JavaScript with Azure OpenAI preserve that prompt and the legacy error-free reply contract, and Python with Azure OpenAI preserves `List Contoso history in table` and the `1985` response assertion. All five cases preserve their work items, provision and deploy before `Launch Remote in Teams (Chrome)`, and compose existing operations without adding capability; the Python Azure OpenAI case does not add a virtual-environment step absent from its legacy flow. |
| VCB-140 | Given the existing Partial OpenAI remote Teams replacements for Weather Agent TypeScript and JavaScript and General Teams Agent TypeScript, JavaScript, and Python, each remote case preserves the legacy fake key through `${{secret:FAKE_OPENAI_API_KEY}}` without changing its local counterpart, provisions and deploys, launches its language-specific remote Teams profile, and sends the legacy prompt. Weather cases assert `encountered an error`, General TypeScript and JavaScript assert `The agent encountered an error or bug.`, and General Python preserves its legacy send-only contract; all five mappings can therefore become Full using existing operations.  |
