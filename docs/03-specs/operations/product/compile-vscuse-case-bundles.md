...
| VCB-34  | Given the checked-in case sources and no injected `compileStep`, setup reads no external template contracts and uses the semantic compiler plus component renderer to emit ninety-four deterministic current-format runnable plans; every operation resolves through a supported adapter, removed manifest-owned cases are deleted, and a second setup reports no generated-plan changes.                                                                                                                                                                                                                                            |
...
| VCB-125 | Given a Copilot target readiness assertion or Copilot message-input assertion, compilation identifies the visible agent by the underscore-free form of the authored `${{var:app_name}}` prefix, because Microsoft 365 Copilot removes underscores from the displayed agent name while the generated project name retains them.                                                                                                                                                                                                                                                                                                       |
