...
| `steps.<name>.type`                                     | Required; one of `scaffold`, `login`, `provision`, `deploy`, `configureActionAuthentication`, `removeWorkspaceFile`, `target`, `open`, or `checks`.                                                                                   |
...
| `provision.with.oauth`                                  | Existing-API OAuth credentials: `clientId` uses `${{env:NAME}}`; `clientSecret` uses `${{secret:NAME}}` and is required unless a preceding `configureActionAuthentication` selected PKCE.                                             |
| `provision.with.microsoftEntra`                         | Existing-API Microsoft Entra credentials after matching authentication configuration: `clientId` uses `${{env:NAME}}`.                                                                                                                |
...
| `configureActionAuthentication.with.authName`           | Required non-empty authentication configuration name.                                                                                                                                                                                 |
| `configureActionAuthentication.with.authType`           | Required stable authentication type ID: `api-key`, `bearer-token`, `microsoft-entra`, or `oauth`.                                                                                                                                     |
| `configureActionAuthentication.with.apiKeyIn`           | Required only for `api-key`; stable request location ID `header` or `query`.                                                                                                                                                          |
| `configureActionAuthentication.with.apiKeyName`         | Required non-empty API-key name only for `api-key`.                                                                                                                                                                                   |
| `configureActionAuthentication.with.authorizationUrl`   | Required absolute URL only for `oauth`.                                                                                                                                                                                               |
| `configureActionAuthentication.with.tokenUrl`           | Required absolute URL only for `oauth`.                                                                                                                                                                                               |
| `configureActionAuthentication.with.refreshUrl`         | Optional absolute URL only for `oauth`; omission submits the product's accepted empty value.                                                                                                                                          |
| `configureActionAuthentication.with.scope`              | Required non-empty scope only for `microsoft-entra` and `oauth`.                                                                                                                                                                      |
| `configureActionAuthentication.with.pkce`               | Required boolean only for `oauth`.                                                                                                                                                                                                    |
...
`localEnvironment`, `localUserEnvironment`, `configureActionAuthentication`,
`removeWorkspaceFile`, `target`, `open`, and `checks`.
...
`configureActionAuthentication` runs the toolkit's dedicated add-authentication command against
the scaffolded project's default `appPackage/ai-plugin.json`. Its `with` object is a closed
discriminated union keyed by `authType`: `api-key` additionally requires `apiKeyIn` and
`apiKeyName`; `bearer-token` accepts no type-specific fields; `microsoft-entra` requires `scope`;
and `oauth` requires `authorizationUrl`, `tokenUrl`, `scope`, and boolean `pkce`, with an optional
`refreshUrl`. The compiler owns command and prompt titles, visible option labels, the default
plugin-manifest selection, and the success notification. It rejects missing, extra, empty, or
type-incompatible inputs before emitting interactions.
For the existing-API template, a later `provision.with.microsoftEntra` submits the Entra client ID
only when that authentication type was configured. `provision.with.oauth.clientSecret` remains
required for ordinary OAuth, while a preceding OAuth configuration with `pkce: true` permits the
client-ID-only prompt sequence the product exposes.
...
| VCB-05  | Given `provision.with.arm`, compilation includes the supported ARM questions and requires every supported ARM input; given `provision.with.oauth` for `da/api-plugin-from-existing-api`, compilation requires environment/secret credential expressions and emits its recorded prompts and confirmation, except that PKCE configured by a preceding authentication operation requires only its environment client ID; given matching `provision.with.microsoftEntra`, compilation requires only its environment client ID; other templates or unmatched configurations reject these inputs. |
...
| VCB-34  | Given the checked-in case sources and no injected `compileStep`, setup reads no external template contracts and uses the semantic compiler plus component renderer to emit ninety-seven deterministic current-format runnable plans; every operation resolves through a supported adapter, removed manifest-owned cases are deleted, and a second setup reports no generated-plan changes.                                                                                                                                                                                                  |
...
| VCB-44  | Given a Copilot `chat` check, the message-input assertion and the click that focuses it read the input as a `Message` composer and identify the page by the app prefix the case authored, never by the full placeholder text, because Microsoft 365 Copilot serves the previewed agent page in variants that put the agent name in the placeholder in one and leave the generic `Message Copilot` placeholder with the agent named above the composer in another.                                                                                                                           |
...
| VCB-95  | Given the General Teams Agent bundle, every LLM service and supported language is authored against remote and local Teams, every language is authored against Agents Playground with Azure OpenAI, and TypeScript with Azure OpenAI samples remote and local Copilot, without crossing every language through the equivalent Copilot surface.                                                                                                                                                                                                                                               |
...
| VCB-97  | Given a General Teams Agent OpenAI case, a local Teams launch writes `OPENAI_BASE_URL` into the local lifecycle and asserts a chat reply, while a remote Teams launch stops after app readiness because its remote configuration carries no compatible base URL.                                                                                                                                                                                                                                                                                                                            |
...
| VCB-107 | Given `localUserEnvironment` with shell-safe non-empty names and values, compilation emits one code step per entry that replaces exactly one existing assignment in `env/.env.local.user`, verifies the written value without logging it, and accepts no authored file path; invalid input fails before plan output.                                                                                                                                                                                                                                                                        |
...
| VCB-121 | Given the JavaScript default-bot case that reuses an existing bot registration, compilation writes `BOT_ID` and `SECRET_BOT_PASSWORD` through `localEnvironment` before starting the local Teams target, then opens the app and retains the echo-response check.                                                                                                                                                                                                                                                                                                                            |
| VCB-122 | Given declarative-agent cases that add API-key, bearer-token, Microsoft Entra, OAuth, or PKCE OAuth authentication after scaffolding an unauthenticated existing API, compilation selects the default plugin manifest, emits only the prompts required by the authored stable auth type and its closed input shape, waits for the project-configuration success notification, and keeps each case's provision and Copilot sign-in validation; missing, extra, empty, or type-incompatible authentication inputs fail before plan output.                                                    |
