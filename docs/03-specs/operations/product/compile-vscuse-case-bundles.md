...
| VCB-139 | Given the retained RAG Customize remote Teams scenarios, TypeScript and JavaScript with OpenAI reuse the Azure OpenAI key and compatible v1 endpoint and preserve the `Compare Contoso Electronics plan` prompt, TypeScript and JavaScript with Azure OpenAI preserve that prompt and the legacy error-free reply contract, and Python with Azure OpenAI preserves `List Contoso history in table` and the `1985` response assertion. All five cases preserve their work items and provision and deploy before `Launch Remote in Teams (Chrome)`; the Python Azure OpenAI case does not add a virtual-environment step absent from its legacy flow.                                                                               |
...
| VCB-142 | Given any generated OpenAI case, its scaffold supplies `${{secret:AZURE_OPENAI_API_KEY}}`; a local launch writes `OPENAI_BASE_URL=${{env:AZURE_OPENAI_ENDPOINT}}/openai/v1` through `localEnvironment`, while a remote launch uses `remoteEnvironment` to insert the same setting into the generated App Service Bicep `appSettings` before provision. If the selected template hardcodes an OpenAI model other than `gpt-4o` or `gpt-4o-mini`, an `openAIModel` operation replaces exactly one authored occurrence with `gpt-4o-mini` and verifies the generated file; templates already using either supported model emit no model mutation. No generated OpenAI plan contains a fake key or asserts a fake-key error response. |
