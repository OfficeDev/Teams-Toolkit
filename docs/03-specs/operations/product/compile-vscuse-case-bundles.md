...
| `provision.with.environment` | Optional literal `none`, declaring that the template scaffolds a single selectable environment so the toolkit auto-selects it and shows no picker. Orthogonal to the input groups above; omitting it emits the recorded `dev` selection. |
...
Environment selection is emitted unless the step declares `with.environment: none`, which records
that the template scaffolds a single selectable environment so the toolkit auto-selects it. V1
