...
| VCB-154 | Given the seven named legacy plans, six new lowercase `feature-*.yml` sources and one existing `non-sso-tab.yml` case generate complete replacements, and their mapping rows are Full. The mislabeled `DA_No_Action_Add_Knowledge_Onedrive.json` and obsolete generic `DA_Add_Action_Import_Existing_API.json` plans are removed with Retired mapping rows because their retained behavior is covered elsewhere. At this migration checkpoint, `Basic_Tab_Remote_Debug.json` remains Partial because its provision-before-login path is not represented; VCB-180 subsequently replaces that path with a capability case. Other blocked plans retain evidence-backed Not Mapped rows until later criteria replace or retire them, and generation is idempotent.                                                                                                                                                                          |
