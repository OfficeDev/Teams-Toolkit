...
| VCB-98 | Given a General Teams Agent case, the plan enables the `M365AgentsToolkit.enableLaunchAgentForTeamsInCopilot` VS Code setting and reloads the workbench before scaffolding, because the extension loads that setting into `TEAMSFX_CEA_ENABLED` during activation and the template renders its Copilot launch profiles only when the flag is enabled.                                                                                                                                                                                                                              |
| VCB-99 | Given an Agents Playground chat check that expects a reply, the completion assertion names the visible non-empty assistant response, its thumbs-up and thumbs-down feedback controls, the composer ready for the next user turn, and the absence of a response-generation indicator, so the ready composer is treated as completion evidence rather than an unfinished turn.                                                                                                                                                                                                       |
