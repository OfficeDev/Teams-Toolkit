...
| CCI-03  | L1   | `entryParams={ mcpServerUrl:<url> }`, remote-only provider, a scripted UI answering `authType=oauth` or `authType=entra-sso`                                                           | `runCreateInputs`                                                           | `mcpServerUrl` is taken from the seed (**not** prompted), the auth variant flows through unchanged, and no client id, client secret, or scope is prompted or returned; credential collection belongs to the first `oauth/register` provision run                                                                                                                                                                     |
...
| CCI-09  | L1   | the in-memory floor                                                                                                                                                                    | `openCreateQuestions(floor, { kind:"create", templateId:"da/mcp-server" })` | `ok([mcpServerType, mcpServerUrl, selectedLocalServers, authType])`; no credential material is authored as a create question; an unknown `templateId` is a `SystemError` named `PackageFileMissing`                                                                                                                                                                                                                  |
...
- **INV-6** — The bridge threads the caller's 1-based `step` onto each host config
  ...
  INPUT-24).
- **INV-7** — MCP create questions may select an authentication mode but never
  collect client ids, client secrets, or scopes. Static OAuth and Entra
  credentials are provision-time inputs owned by the existing
  `oauth/register` question middleware; no credential value enters `Answers` or
  scaffold rendering.
