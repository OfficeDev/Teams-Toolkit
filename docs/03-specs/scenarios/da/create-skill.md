...
| SCN-CREATE-SKILL-03 | L1   | rendered `appPackage/manifest.json`                    | render             | `manifestVersion == "1.29"`; the env refs survive render - `id == "${{TEAMS_APP_ID}}"`, `name.short == "{{appName}}${{APP_NAME_SUFFIX}}"`; `copilotAgents.declarativeAgents` is the single entry `{ id: "declarativeAgent", file: "declarativeAgent.json" }` |
