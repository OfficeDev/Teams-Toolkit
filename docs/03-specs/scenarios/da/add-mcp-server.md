...
| SCN-ADD-MCP-15 | L1   | `authType=bearer-token`                                                                                                           | render + steps       | plugin `auth.type == "ApiKeyPluginVault"` and `reference_id == mcpAuthRef(mcpServerUrl)`; `mcp-auth/inject-yml-action` injects `apiKey/register` with `baseUrl=mcpServerUrl` and no `apiSpecPath`                                                                                                                        |
