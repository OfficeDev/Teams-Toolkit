...
| SCN-CREATE-TYPESPEC-04 | L1 | rendered `appPackage/manifest.json` | render | `manifestVersion == "1.30"`; `id == "${{TEAMS_APP_ID}}"`; `name.short == "{{appName}}${{APP_NAME_SUFFIX}}"`; TypeSpec-owned declarative-agent output is not pre-baked in the manifest |
