...
| SCN-CREATE-RAG-AZURE-SEARCH-04 | L1      | scenario | per-PR | InMemoryRuntime | Scaffold the Python Azure AI Search RAG agent.                                 | The scaffold selects the Python subtree, writes Python app and indexer files, omits Node package files, and catches OpenAI API failures from search-context generation or prompt sending inside the bot flow so they produce a safe log message and fallback response instead of an unhandled 500. |
