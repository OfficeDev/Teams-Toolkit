...
| SCN-CREATE-RAG-CUSTOM-API-05 | L1      | scenario              | per-PR | InMemoryRuntime | Scaffold the Python custom API RAG agent.                                                     | The scaffold selects the Python subtree, injects Python function wiring for the selected API operation, writes bot prompt suggestions from the selected operation `summary` into the Teams manifest (falling back to `description` only when `summary` is absent), and catches OpenAI API failures inside the bot flow so they produce a safe log message and fallback response instead of an unhandled 500. |
