...
New projects now target the Teams app manifest schema v1.28 by default, and the manifest parser supports both v1.27 and v1.28. The update relaxes URL validation for developer fields and composeExtensions.commands[].taskInfo.url, allowing HTTP in addition to HTTPS where permitted by the schema. Existing projects continue to work, while new projects benefit from the more flexible validation rules and updated converters. This alignment reduces friction when authoring or validating manifests and ensures compatibility with the latest schema. You can continue to package and validate from VS Code or the CLI as before—no extra steps required.
