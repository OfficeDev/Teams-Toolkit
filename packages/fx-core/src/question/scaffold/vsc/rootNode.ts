...
function loadUiNode(fileName: string, platform: Platform): IQTreeNode {
  ...
  // option ids, and `onDidSelection` callbacks. A downloaded `~/.fx/ui` copy can
  // lag the local code (a dev build ahead of the published v4 metadata, or a
  // leftover cache from an earlier version). On the v4 channel, pin the UI tree to
  // the bundled artifact that ships with this fx-core. (Metadata descriptors keep
  // the `useBundledMetadataForV4`
