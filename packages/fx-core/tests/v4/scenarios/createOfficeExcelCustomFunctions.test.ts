...
/**
 * T3 scenario tier: the `office-addin-excel-customfunctions` create package scaffolded under
 * `InMemoryRuntime`.
 *
 * Spec: docs/03-specs/scenarios/office/create-excel-customfunctions.md
 * (SCN-CREATE-EXCEL-CUSTOMFUNCTIONS-01..04)
 */

const templatePackage = loadV4Package("create", "office-addin-excel-customfunctions");
...
describe("SCN-OFFICE-CREATE-EXCEL-CUSTOMFUNCTIONS (v4, T3 InMemoryRuntime)", () => {
  it("SCN-CREATE-EXCEL-CUSTOMFUNCTIONS-01: scaffold writes the Excel add-in file set", async () => {
    ...
    assert.include(outcome.written, "src/functions/functions.ts");
    assert.include(outcome.written, "infra/azure.bicep");
    assert.include(outcome.written, "webpack.config.js");
  });

  it("SCN-CREATE-EXCEL-CUSTOMFUNCTIONS-02: package and manifest render appName-derived values", async () => {
  ...
  it("SCN-CREATE-EXCEL-CUSTOMFUNCTIONS-03: only require-empty-target runs", async () => {
    const { outcome } = await run();
    assert.deepStrictEqual(outcome.stepsRun, ["require-empty-target"]);
  });

  it("SCN-CREATE-EXCEL-CUSTOMFUNCTIONS-04: a non-empty target fails require-empty-target first", async () => {
